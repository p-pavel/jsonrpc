...
trait MapLock[F[_], K, V]:
  /** @param action
    ...
    */
  def reserve(key: K, action: F[Unit]): F[Option[V]]
    ...
    Ref
      ...
      .map(refMap =>
        new MapLock[F, K, V]:
          ...
          override def reserve(key: K, action: F[Unit]): F[Option[V]] =
            ...
            OptionT(allocDeferred)
              .semiflatMap(a => action.as(a))
              .semiflatMap(processDeferred)
              .value
